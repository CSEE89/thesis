...
namespace lemon {
  ...
  public:
	 ...
	 // typedef typename GR::template Path<GR> listpath; //tipusdefiniciók a ModDijkstra tárolási egységeinek
	  //typedef std::pair<listpath, SpectrumState> pathpair;
	  //typedef std::vector<pathpair> pathpair_vector;

	  template<typename T> struct maki{
		  typedef lemon::Path<T> path_type;
	  };
	  typedef typename maki<GR>::path_type listpath2;
	  typedef std::pair<listpath2, SpectrumState> pathpair2;
	  //typedef std::vector<pathpair> pathpair_vector;
	  typedef typename GR::template EdgeMap<SpectrumState> spectrum_stateMap;
	  typedef typename GR::template NodeMap<pathpair_vector> pathpair_vector_NodeMap;
	  typedef typename GR::template EdgeMap<bool> permitting_edgeMap;
    ...
    ///\param length The length map used by the algorithm.
	  Dijkstra(const Digraph& g, LengthMap& length, ListGraph::EdgeMap<SpectrumState> &st) :
		G(&g), _length(&length),spectrum_map(st),
    ...
    { }
	 Dijkstra(const Digraph& g,LengthMap& length) :
		G(&g), _length(&length),
      _pred(NULL), local_pred(false),
      _dist(NULL), local_dist(false),
      _processed(NULL), local_processed(false),
  ...
  public:
	...
	bool p_contain(pathpair &t,Arc u,const Node &w)
	{
		lemon::Path<ListGraph>::ArcIt path_arcit(t.first);
		for(path_arcit;path_arcit!=INVALID;++path_arcit)
			{
				if(G->id(path_arcit)==G->id(u))
					return true;
			}
		for (path_arcit; path_arcit != INVALID; ++path_arcit)
		{
			if (G->target(path_arcit) == w || G->source(path_arcit) == w)
					return true;
		}
		//cout << typeid(*G).name();
		//PathNodeIt<	  maki<GR>::path_type > PN(*G,t.first);  // ITT ereddetileg ListGraph ------>>>>  MIÉRT NEM JÓ????
			//for(PN;PN!=INVALID;PN++)
				//{
					//if(G->id(PN)==G->id(w)) return true;
				//}
		return false;
	}
...
//-----------permitting map elõszûrést csinál, az eleve kizárt éleket(ahol semelyik sávban nincs elég hely) kiejtjük    
	...
	Node modprocessNextNode(permitting_edgeMap &permittingmap, pathpair_vector_NodeMap &pathmap)
	{
      ...
      for(OutArcIt e(*G,v); e!=INVALID; ++e) {
        Node w=G->target(e);
		ListGraph::Edge edge = lemon::findEdge(*G, v, w);

		if(permittingmap[edge])
		{
		level.push_back(w);
    ...
    ///  }
    ///\endcode
	void start(permitting_edgeMap &permittingmap)
    {
